...
module.exports = {
	...
	async execute(message, args, server) {
        ...
        await changeGuildConfig(server, 'rosterName', rosterSheetNameMessage.content).then( async() => {
        ...
        await changeGuildConfig(server, 'nameHeader', nameHeaderMessage.content).then( async() => {
        ...
        await changeGuildConfig(server, 'rankHeader', rankHeaderMessage.content).then( async() => {
        ...
        await changeGuildConfig(server, 'statusHeader', statusHeaderMessage.content).then( async() => {
        ...
        await changeGuildConfig(server, 'discordHeader', discordIdHeaderMessage.content).then( async() => {
...
async function requestMessageInput(originalCommandMessage, requestMessage) {
    ...
    const response = new Promise(async (resolve, reject) => {
        await originalCommandMessage.channel.send(requestMessage).then(async () => {
            await originalCommandMessage.channel.awaitMessages(filter, {
                max: 1,
                time: 60000,
                errors: ['time']
            }).then((message) => {
                ...
                return originalCommandMessage.channel.send('Timed out! Please restart the initialization proccess.');
            })
        })
    })
    return response;
